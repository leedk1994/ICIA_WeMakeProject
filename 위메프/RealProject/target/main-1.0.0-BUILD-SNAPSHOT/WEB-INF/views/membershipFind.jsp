    					...
    					<tr>
    						...
    						<td>
    							<span id="idFind">
    								<label>
    									아이디 : 
    									<input type="text" class="inputTxt01" id="userId" name="userId" placeholder="아이디" required="required" style="width:110px;"/>
    								</label>
    								<label>
    									이름 : 
			...
			$.ajax({
				...
				success:function(d){
					...
					if(reqUrl=="./membershipController/findId"){
						...
						if(d.pwok == "Y"){
							window.open("membershipRePw?user_id="+$("#userId").val()+"","비밀번호 재설정","width=450, height=400, toolbar=no, menubar=no, scrollbars=no, resizable=yes, top="+(screen.availHeight/2-100)+",left="+(screen.availWidth/2-100)+"");
							$("#user_id").val("");
							$("#user_name2").val("");
							$("#phone2").val("");
							$("#user_name").val("");
